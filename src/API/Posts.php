...
class Posts extends AbstractAPI
{
    ...
    {
        return $this->client->post('wp.getPost', compact('blog_id', 'fields'));
    }

    public function getPosts($blog_id, $filter = [], $fields = []): HttpResponse
    {
        return $this->client->post('wp.getPosts', compact('blog_id', 'filter', 'fields'));
    }

    public function newPost($blog_id, $content): HttpResponse
    {
        return $this->client->post('wp.newPost', compact('blog_id', 'fields'));
    }

    public function editPost($blog_id, $post_id, $content): HttpResponse
    {
        return $this->client->post('wp.editPost', compact('blog_id', 'post_id', 'content'));
    }

    public function deletePost($blog_id, $post_id): HttpResponse
    {
        return $this->client->post('wp.deletePost', compact('blog_id', 'post_id'));
    }

    public function getPostType($blog_id, $post_type_name, $fields = []): HttpResponse
    {
        return $this->client->post('wp.getPostType', compact('blog_id', 'post_type_name', 'fields'));
    }

    public function getPostTypes($blog_id, $filter = [], $fields = []): HttpResponse
    {
        return $this->client->post('wp.getPostTypes', compact('blog_id', 'filter', 'fields'));
    }

    public function getPostFormats($blog_id, $filter = []): HttpResponse
    {
        return $this->client->post('wp.getPostFormats', compact('blog_id', 'filter'));
    }

    public function getPostStatusList($blog_id): HttpResponse
    {
        return $this->client->post('wp.getPostStatusList', compact('blog_id'));
    }

    public function getRevisions($blog_id, $post_id): HttpResponse
    {
        return $this->client->post('wp.getRevisions', compact('blog_id', 'post_id'));
    }

    public function restoreRevision($blog_id, $revision_id): HttpResponse
    {
        return $this->client->post('wp.restoreRevision', compact('blog_id', 'revision_id'));
    }
}
