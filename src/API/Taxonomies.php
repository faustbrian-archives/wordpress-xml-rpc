...
class Taxonomies extends AbstractAPI
{
    ...
    {
        return $this->client->post('wp.getTaxonomy', compact('blog_id', 'taxonomy'));
    }

    public function getTaxonomies($blog_id): HttpResponse
    {
        return $this->client->post('wp.getTaxonomies', compact('blog_id'));
    }

    public function getTerm($blog_id, $taxonomy, $term_id): HttpResponse
    {
        return $this->client->post('wp.getTerm', compact('blog_id', 'taxonomy', 'term_id'));
    }

    public function getTerms($blog_id, $taxonomy, $filter = []): HttpResponse
    {
        return $this->client->post('wp.getTerms', compact('blog_id', 'taxonomy', 'filter'));
    }

    public function newTerm($blog_id, $content = []): HttpResponse
    {
        return $this->client->post('wp.newTerm', compact('blog_id', 'fields'));
    }

    public function editTerm($blog_id, $term_id, $content): HttpResponse
    {
        return $this->client->post('wp.editTerm', compact('blog_id', 'term_id', 'content'));
    }

    public function deleteTerm($blog_id, $taxonomy, $term_id): HttpResponse
    {
        return $this->client->post('wp.deleteTerm', compact('blog_id', 'taxonomy', 'term_id'));
    }
}
