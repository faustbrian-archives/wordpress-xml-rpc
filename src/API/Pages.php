...
class Pages extends AbstractAPI
{
    ...
    {
        return $this->client->post('wp.getPage', compact('blog_id', 'page_id'));
    }

    public function getPages($blog_id, $max_pages = 10): HttpResponse
    {
        return $this->client->post('wp.getPages', compact('blog_id', 'max_pages'));
    }

    public function getPageList($blog_id): HttpResponse
    {
        return $this->client->post('wp.getPageList', compact('blog_id'));
    }

    public function newPage($blog_id, $content, $enclosure, $publish): HttpResponse
    {
        return $this->client->post('wp.newPage', compact('blog_id', 'content', 'enclosure', 'publish'));
    }

    public function editPage($blog_id, $page_id, $content, $publish): HttpResponse
    {
        return $this->client->post('wp.editPage', compact('blog_id', 'page_id', 'content', 'publish'));
    }

    public function deletePage($blog_id, $page_id): HttpResponse
    {
        return $this->client->post('wp.deletePage', compact('blog_id', 'page_id'));
    }

    public function getPageStatusList($blog_id): HttpResponse
    {
        return $this->client->post('wp.getPageStatusList', compact('blog_id'));
    }

    public function getPageTemplates($blog_id): HttpResponse
    {
        return $this->client->post('wp.getPageTemplates', compact('blog_id'));
    }
}
